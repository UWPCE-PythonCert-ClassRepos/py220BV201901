# Parallel
""""
Module to handle parallel database operations
"""
...
async def import_data(directory_name, product_file, customer_file, rental_file):
    """ create tasks to run in parallel """
    prdct_result = asyncio.create_task(import_product(directory_name, product_file))
    cust_result = asyncio.create_task(import_customer(directory_name, customer_file))
    rntl_result = asyncio.create_task(import_rental(directory_name, rental_file))
    result = await asyncio.gather(prdct_result, cust_result, rntl_result)
    return result


async def import_product(directory_name, product_file):
...
async def import_rental(directory_name, rental_file):
...
if __name__ == "__main__":
    ...
    # Setup and enable internal profiling
    PR = cProfile.Profile()
    PR.enable()

    # Start parallel operations and measure total module run time
