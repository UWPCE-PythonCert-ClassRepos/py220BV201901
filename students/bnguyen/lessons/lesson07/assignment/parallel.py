...
async def process_csv_basic(csv_file_in):
    """
...
async def update_db(collection_name, dir_name, file_name, key_name=None, key_name_1=None):
    """
    ...
    # csv_task = asyncio.create_task(process_csv_basic(file_path_name))
    # csv_list = await asyncio.gather(csv_task)
    id_coro = process_csv_basic(file_path_name)
    csv_list = await id_coro
    ...
    await asyncio.sleep(0)  # Still working on learning this!
...
async def import_data(directory_name, product_file, customer_file, rentals_file):
    ...
    products_task = asyncio.create_task(update_db('products', directory_name, 
                                                  product_file, 'product_id'))
    customers_task = asyncio.create_task(update_db('customers', directory_name,
                                                   customer_file, 'Id'))
    rentals_task = asyncio.create_task(update_db('rentals', directory_name,
                                                 rentals_file, 'user_id', 'product_id'))
    results = await asyncio.gather(products_task, customers_task, rentals_task)
...
def main():
    ...
    result_out = asyncio.run(import_data(data_dir, 'products.csv', 'customers.csv', 'rentals.csv'))
    print(result_out)
