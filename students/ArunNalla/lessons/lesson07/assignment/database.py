...
async def product_csv(file_name):
    ...
    try:
        ...
        # print (time.time() - start_time, "Time to complete product file")
...
async def customers_csv(file_name):
    ...
    try:
        ...
        print (elasped_time, "Time taken for reading customer file")
...
async def rentals_csv(file_rent):
    ...
    try:
        ...
        print (time.time()-start_time, 'Time taken for reading rental file')
        return (rent_added[0], rent_errors[0])
    except FileNotFoundError:
...
async def import_data(DATA_DIR, products, customers, rentals):
    ...
    print (tuple(zip(*final_result)))
    print ()
    print (time.time()-start_time, "TOTAL TIME")
    return tuple(zip(*final_result))
