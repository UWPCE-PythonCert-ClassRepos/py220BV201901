...
def parallel():
    ...
    cust_thread = threading.Thread(target=ingest_customer_csv, args=CSV_PATH_DBG + CUST_CSV_FILENAME, True)
    prod_thread = threading.Thread(target=ingest_product_csv, args=CSV_PATH_DBG + PROD_CSV_FILENAME, True)
    rent_thread = threading.Thread(target=ingest_rental_csv, args=CSV_PATH_DBG + RNTL_CSV_FILENAME, True)
